---
name: ho-project-setup-collaborator
description: >
  ...
  `ho-environment-setup-collaborator`, which handles the practitioner-scope `~/.claude/`
...
If these don't exist, redirect: "It looks like your Claude Code environment isn't configured for Ho System yet. Run the `ho-environment-setup-collaborator` skill first—it sets up `~/.claude/` with the operating discipline, language modules, and the per-project templates this skill uses. Once that's done, come back here and we'll scaffold the project."
...
```bash
git commit -m "Initial scaffold via ho-project-setup-collaborator"
```
...
**Scaffolding before the project type is clear.** If the practitioner doesn't know whether they're building a service or a library, they don't know what to scaffold. Send them to `ho-seed-collaborator` first.
...
`ho-environment-setup-collaborator` runs once per practitioner. It produces the `~/.claude/` config that this skill assumes. If the practitioner doesn't have that environment yet, send them there first.
