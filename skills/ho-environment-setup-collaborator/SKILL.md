---
name: ho-environment-setup-collaborator
description: >
...
```bash
ln -s <framework-path>/practitioner/the-operating-discipline.md \
...
`ho-project-setup-collaborator` runs once per new project. It uses the templates this skill places at `~/.claude/templates/` to scaffold individual projects with the conventions encoded here. Run that skill when a practitioner says "I'm starting a new project."
