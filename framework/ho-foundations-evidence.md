...
The Ho System's structural answer to the overconfidence problem documented in the METR and CodeRabbit research is the five-layer verification stack: automated tests, linting, directed self-review, cross-agent verification, and human architectural review. That stack answers one question — was the system built *right*? — which is only half of quality; its complement is the **validation** layer ([[verification-practices|Verification Practices §3]] (framework/structure/verification-practices.md)), which asks whether the *right thing* was built and whether it is good — extending this same overconfidence antidote from correctness into goodness. Verification is not theoretical hygiene — it is the mechanism that makes "architectural authority" real rather than aspirational. An architect who cannot verify the build is not an architect; they are an optimist with a deploy button. See [[verification-practices|Verification Practices]] (framework/structure/verification-practices.md) for the complete framework.
